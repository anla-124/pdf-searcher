...
WHERE status IN ('processing', 'queued', 'uploading', 'cancelling');
...
CREATE INDEX IF NOT EXISTS idx_extracted_fields_page ON extracted_fields(document_id, page_number);
...
CREATE POLICY "Users can view own extracted fields" ON extracted_fields FOR SELECT 
USING (EXISTS (SELECT 1 FROM documents d WHERE d.id = document_id AND d.user_id = auth.uid()));

DROP POLICY IF EXISTS "Users can insert own extracted fields" ON extracted_fields;
CREATE POLICY "Users can insert own extracted fields" ON extracted_fields FOR INSERT 
