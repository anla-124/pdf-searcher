...
export async function stage2FinalScoring(
  ...
  try {
    ...
    // 1. Fetch source chunks once (reuse for all candidates)
...
async function processCandidate(
  ...
  // 3. Adaptive scoring (CRITICAL: Use actual chunk counts, not effective!)
  // We fetch ALL chunks for matching, so we must use actual counts in scoring
  logger.debug('Stage 2: computing score for candidate', {
    candidateId,
    sourceChunkCount: sourceChunks.length,
...
async function fetchDocumentChunks(documentId: string): Promise<Chunk[]> {
  ...
  while (true) {
    ...
    const { data, error } = await supabase
      ...
      .select('chunk_index, page_number, embedding, chunk_text')
